...
p = Plots.boxplot(["S-S" "Nash competition, N-N" "Bilevel competition, L-F "],
    [
...
annotate!([(0.2, .145, Plots.text(L"\times10^{-2}", 12, :black, :center))])
Plots.plot!(p, size=(500, 400), xlabel="Competition type", ylabel="Mean velocity cost per simulation step", yaxis=(formatter = y -> round(y*100; sigdigits=4)))
