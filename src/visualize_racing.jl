...
function update_visual!(ax, XA, XB, x0, P1, P2; T=10, lat=6.0, x_lims=nothing, y_lims=nothing)
    XA[0][1][] = x0[1]
    XA[0][2][] = x0[2]
    XB[0][1][] = x0[5]
    XB[0][2][] = x0[6]
    ...
    if isnothing(y_lims)
        ...
        y_lims = (avg_y_pos - lat, avg_y_pos + 3 * lat)
    end
...
function randomly_animate(mode, results, roads, params, sample_size; sample=nothing, t=nothing)
    ...
    if isnothing(t)
        ...
        (f, ax, XA, XB, lat) = visualize_clean(road; rad=params.r / 2, lat=5.1, d=params.d)
        update_visual!(ax, XA, XB, x0, P1, P2; T=params.T, lat=lat)
        #(f, ax, XA, XB, lat) = visualize_clean(road; rad=params.r / 2, lat=5.1, d=params.d, car_marker_size=40, size=(500,500))
        #update_visual!(ax, XA, XB, x0, P1, P2; T=params.T, lat=lat, x_lims=(-5.5, 4.5), y_lims=(8,18))
        #@infiltrate
        GLMakie.activate!()
        display(f)
        CairoMakie.activate!()
        CairoMakie.save("output/sample.pdf", f)
    end
end
